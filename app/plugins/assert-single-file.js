...
const assertSingleFile = () => ({
  ...
  generateBundle: (unusedOptions, bundle) => {
    ...
    if (emittedFiles.length !== 1) {
      throw new Error(
        `slopmap: expected one emitted file, received ${emittedFiles.length}`
      );
    }
    ...
    if (
      ...
      !emittedFile.fileName.endsWith('.html')
    ) {
    ...
    if (countOccurrences(html, '__SLOPMAP_DATA__') !== 1) {
      throw new Error(
        'slopmap: viewer bundle must contain exactly one payload marker'
      );
    }
    if (countOccurrences(html, '__SLOPMAP_TITLE__') !== 1) {
      throw new Error(
        'slopmap: viewer bundle must contain exactly one title marker'
      );
    }
    if (html.includes('src="')) {
      throw new Error(
        'slopmap: viewer bundle contains an external script source'
      );
    }
    if (html.includes('href="./assets')) {
      throw new Error(
        'slopmap: viewer bundle contains an emitted asset reference'
      );
    }
    if (html.includes('__vitePreload')) {
      throw new Error(
        'slopmap: viewer bundle contains the Vite preload helper'
      );
    }
